...
import Engine.RSM.ManagerState;
 ...
 *
 *
 * Behaviors: 1) Switch states between load and game, and change load time limit
 * based on that 2) Load and store resources. Store static resources
 * indefinitely, store dynamic resources based on pointer count. 3) Quickly give
...
public final class RSM {
     ...
     */
    private static double maxLoadTime = 1;
     ...
     */
    private RSM() throws InstantiationException {
    ...
    public static void notifyUnload(HashMap<String, String> manifest) {
        RSM.eventQueue.addLast(new unloadMessage(manifest));
    }
    ...
    public static void notifyLoad(HashMap<String, String> manifest) {
        RSM.eventQueue.addLast(new loadMessage(manifest));
    }
    ...
    private static void allocate(String resourceID, String resourcePath) {
        if (textures.get(resourceID) != null) {
            ...
            // hardcode for now, to load textures.
            // Assumes everything is a texture
                RSM.loadTextureResource(resourceID, resourcePath);
     ...
     * @return the texture id or -1 if it is not found
     */
    public static int getTextureResource(String id) {
        int texture = staticTextures.getOrDefault(id, -1); // -1 is null
        if (texture == -1 && textures.containsKey(id)) {
    ...
    private static int loadTexturePNG(String filename, int textureUnit) {
        ...
        try {
            ...
            // generate some file exception warning. Probably i will put missed
            // files in a list.
            return -1;
        }
     ...
     */
    public static void setLoadState(ManagerState state) {
        RSM.state = state;
        if (state == ManagerState.LOAD_STATE) {
            maxLoadTime = (int) (GameLoop.TICK_RATE / 2);
        }
        if (state == ManagerState.PLAY_STATE) {
            maxLoadTime = (int) (GameLoop.TICK_RATE / 16);
    ...
    private static class loadMessage extends Message {
        ...
        public void execute() {
            for (String key : resourceManifest.keySet()) {
                RSM.allocate(key, resourceManifest.get(key));
            }
        }
    }
